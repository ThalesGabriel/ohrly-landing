  ...
  return (
    <article className="min-h-screen bg-[#f7fafc] text-slate-950">
      <section className="mx-auto max-w-7xl px-6 pb-8 pt-10 lg:px-8 lg:pt-14">
        <Link
          href="/estudos"
          className="mb-8 inline-flex items-center gap-2 text-sm font-black text-teal-800 transition hover:text-teal-950"
        >
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-stretch">
          <div className={`relative overflow-hidden rounded-[2rem] bg-gradient-to-br ${study.visualClass} p-8 text-white shadow-xl shadow-slate-900/10`}>
            ...
            <div className="relative z-10 flex h-full min-h-[360px] flex-col justify-between">
              <div>
                <span className="inline-flex items-center rounded-full bg-white/10 px-3 py-1 text-sm font-black text-white">
                  {study.type}
                </span>
          ...
          <header className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8 lg:p-10">
            ...
            <h1 className="mt-6 text-4xl font-black tracking-[-0.045em] text-[#06183d] sm:text-5xl">
              {study.title}
            </h1>
      ...
      <section className="mx-auto grid max-w-7xl grid-cols-1 gap-8 px-6 py-8 lg:grid-cols-[280px_1fr] lg:px-8">
        <aside className="lg:sticky lg:top-24 lg:h-fit">
          <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="text-sm font-black uppercase tracking-[0.18em] text-teal-800">
              Neste estudo
            </h2>
          ...
          <div className="mt-5 rounded-[2rem] border border-cyan-100 bg-cyan-50 p-6">
            ...
            <Link
              href="/avaliador"
              className="mt-5 inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#004653] px-4 text-sm font-black text-white shadow-lg shadow-teal-900/10 transition hover:bg-[#003844]"
            >
              Avaliar fluxo
      ...
      {relatedStudies.length > 0 && (
        <section className="mx-auto max-w-7xl px-6 pb-14 pt-6 lg:px-8">
          <div className="flex items-end justify-between gap-4">
            ...
            <Link
              href="/estudos"
              className="hidden items-center gap-1 text-sm font-black text-teal-800 sm:inline-flex"
            >
              Ver todos
              <ArrowRight className="h-4 w-4" />
            </Link>
...
function StudyBlock({
  ...
  if (block.type === "quote") {
    return (
      <blockquote className="rounded-3xl border-l-4 border-teal-700 bg-teal-50 px-6 py-5 text-xl font-black leading-8 text-[#06183d]">
        {block.content}
      </blockquote>
    );
  }
  ...
  if (block.type === "callout") {
    return (
      <div className="not-prose my-8 rounded-[2rem] border border-cyan-100 bg-cyan-50 p-6">
        <h3 className="text-xl font-black text-[#06183d]">{block.title}</h3>
...
function RelatedStudyCard({ study }: { study: StudyDetail }) {
  ...
  return (
    ...
    >
      ...
      <span className="mt-5 inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-500">
        {study.category}
      </span>

      <h3 className="mt-4 text-lg font-black leading-6 text-[#06183d]">
        {study.title}
      </h3>
      ...
      <span className="mt-5 inline-flex items-center gap-1 text-sm font-black text-teal-800">
        Ler estudo
