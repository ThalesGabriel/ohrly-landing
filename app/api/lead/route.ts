...
const ALLOWED_ATTENTION_METHODS = new Set([
  "health_score",
  "churn_model",
  "rules_alerts",
  "manual_csm",
...
function displayAttentionMethod(value: string) {
  const labels: Record<string, string> = {
    health_score: "Health Score",
    churn_model: "Modelo de churn",
    rules_alerts: "Regras / alertas",
    manual_csm: "CSMs decidem manualmente",
    no_clear_process: "Não há processo claro",
    other: "Outro",
  };
...
export async function POST(request: Request) {
  try {
    ...
    // Honeypot. Retorna sucesso para bots sem persistir nada.
    ...
    const attentionMethod = clean(body.attentionMethod, 60);
    ...
    if (
      ...
      !ALLOWED_ATTENTION_METHODS.has(attentionMethod) ||
    ...
    const visitorId = isUuid(incomingVisitorId)
      ? incomingVisitorId
      : crypto.randomUUID();
    ...
    const attentionMethodLabel = displayAttentionMethod(attentionMethod);
    const customerLabel = displayCustomerCount(customerCount);

    const supabase = getSupabaseAdmin();

    /*
     * Mantemos o schema existente de decision_leads.
     * A nova qualificação comercial vai para context + metadata,
     * sem exigir migration de banco neste momento.
     */
    const { error: leadError } = await supabase
      ...
      .insert({
        ...
        context:
          `Método atual de priorização: ${attentionMethodLabel}. ` +
        ...
        // A proposta deixou de presumir Intercom como sistema obrigatório.
        systems: null,
        ...
        metadata: {
          source: "Ohrly landing page - Account attention",
    ...
    /*
     * Mantemos eventName="form_submit" para preservar continuidade
     * histórica do funil. A nova versão é diferenciada por form_id,
     * landing_variant e attention_method.
     */
    ...
    /*
     * Estes dois helpers também precisam aceitar attentionMethod.
     * A chamada abaixo já representa o contrato novo do formulário.
     */
    const metaPromise = marketingConsent
      ...
      : Promise.resolve({
          ok: false,
          skipped: true,
          reason: "marketing_consent_false",
        } as const);
    ...
    const formspreePromise = sendFormspreeLead({
      ...
      email,
      companySite,
      attentionMethod: attentionMethodLabel,
    ...
    return NextResponse.json({
      ...
      clientEventId,
      metaDelivered:
        metaResult.status === "fulfilled" && metaResult.value.ok,
      formspreeDelivered:
