                    ...
                    <div className="grid grid-cols-1 gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
                        <div>
                            ...
                            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                                <Link
                                    href="#lista"
                                    className="inline-flex h-12 items-center justify-center rounded-xl bg-[#004653] px-6 text-sm font-black text-white shadow-lg shadow-teal-900/10 transition hover:bg-[#003844]"
                                >
                                    Explorar estudos
                                </Link>
                                ...
                                <Link
                                    href="/diagnostic"
                                    className="inline-flex h-12 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-6 text-sm font-black text-[#06183d] shadow-sm transition hover:border-teal-700"
                                >
                            ...
                            {categoryFilters.map((filter) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={[
                                            "inline-flex min-w-fit items-center gap-2 rounded-full border px-4 py-2 text-xs font-black transition",
                                            isSelected
                ...
                <section className="mx-auto max-w-7xl px-6 pb-12 pt-8 lg:px-8">
                    <div className="relative overflow-hidden rounded-[2rem] bg-[#004653] p-8 text-white shadow-xl shadow-teal-950/10 md:p-12">
                        ...
                        <div className="relative z-10 grid grid-cols-1 items-center gap-8 lg:grid-cols-[1fr_auto]">
                            <div>
                                <p className="mb-3 inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-sm font-black text-white">
                            ...
                            <div className="flex flex-col gap-3 sm:flex-row lg:flex-col">
                                <Link
                                    href="/avaliador"
                                    className="inline-flex h-13 items-center justify-center gap-2 rounded-xl bg-white px-6 py-4 text-sm font-black text-[#004653] shadow-lg transition hover:bg-slate-100"
                                >
                                    Avaliar meu fluxo
                                    <ArrowRight className="h-4 w-4" />
                                </Link>
                                ...
                                <Link
                                    href="/contato"
                                    className="inline-flex h-13 items-center justify-center rounded-xl border border-white/20 bg-white/10 px-6 py-4 text-sm font-black text-white transition hover:bg-white/15"
                                >
                                    Solicitar check-up
                                </Link>
                            </div>
...
function HeroMetric({
    ...
    return (
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <p className="mt-3 text-2xl font-black text-[#06183d]">{value}</p>
        ...
        <article className="grid grid-cols-1 overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm lg:grid-cols-[0.95fr_1.05fr]">
            <div className={`relative min-h-[280px] bg-gradient-to-br ${study.visualClass} p-8 text-white`}>
                ...
                <div className="relative z-10 flex h-full flex-col justify-between">
                    <div>
                        <span className="inline-flex items-center rounded-full bg-white/10 px-3 py-1 text-sm font-black">
                            {study.type}
                        </span>
            ...
            <div className="p-6 sm:p-8 lg:p-10">
                <div className="flex flex-wrap gap-2">
                    ...
                    {study.tags.map((tag) => (
                        <span
                            key={tag}
                            className="rounded-full border border-teal-100 bg-teal-50 px-3 py-1 text-xs font-black text-teal-900"
                        >
                            {tag}
                        </span>
                    ))}
                ...
                <h2 className="mt-6 max-w-2xl text-3xl font-black tracking-tight text-[#06183d] md:text-4xl">
                    {study.title}
                </h2>
                ...
                <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
                    {study.status === "new" ? (
                        <Link
                            href={study.href}
                            className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-[#004653] px-6 text-sm font-black text-white shadow-lg shadow-teal-900/10 transition hover:bg-[#003844]"
                        >
                            Ler estudo
                            <ArrowRight className="h-4 w-4" />
                        </Link>
                        ...
                        <button
                            type="button"
                            disabled
                            className="inline-flex h-12 cursor-not-allowed items-center justify-center gap-2 rounded-xl bg-slate-200 px-6 text-sm font-black text-slate-500"
                        >
                            Em produção
                        </button>
                    )}
...
function TrailCard({
    ...
    return (
        ...
        >
            ...
            <h3 className="mt-5 text-xl font-black text-[#06183d]">{trail.title}</h3>
            ...
            <span className="mt-6 inline-flex items-center gap-1 text-sm font-black text-teal-800">
                Explorar trilha
    ...
    return (
        <article className="group flex h-full flex-col overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:border-teal-200 hover:shadow-xl hover:shadow-slate-900/5">
                    ...
                    <div className="flex items-center justify-between gap-4">
                        <div className="flex flex-wrap gap-2">
                            ...
                            {study.status === "in_production" && (
                                <span className="inline-flex items-center gap-1 rounded-full bg-white px-3 py-1 text-xs font-black text-amber-700">
                                    <Clock3 className="h-3.5 w-3.5" />
                                    Em produção
                                </span>
                            )}
            ...
            <div className="flex flex-1 flex-col p-6">
                <span
                    className={[
                        "mb-4 inline-flex w-fit rounded-full border px-3 py-1 text-xs font-black",
                        study.accentClass,
                    ].join(" ")}
                >
                    {study.category}
                ...
                {study.status === "new" ? (
                    <Link
                        href={study.href}
                        className="mt-6 inline-flex items-center gap-1 text-sm font-black text-teal-800"
                    >
                        Ler estudo
                        <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" />
                    </Link>
                    ...
                    <span className="mt-6 inline-flex items-center gap-1 text-sm font-black text-slate-400">
                        Em produção
                        <Clock3 className="h-4 w-4" />
                    </span>
                )}
