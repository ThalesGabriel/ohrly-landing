...
function JourneyStep({ number, title }: { number: string; title: string }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
      <span className="flex h-8 w-8 items-center justify-center rounded-full bg-teal-50 text-sm font-black text-teal-900">
        {number}
      </span>

      <p className="mt-3 text-sm font-black text-[#06183d]">{title}</p>
    </div>
  );
}
...
function PricingCard({ plan }: { plan: PricingPlan }) {
  ...
  return (
    ...
    >
      {plan.badge && (
        <span className="absolute right-6 top-6 rounded-full bg-[#004653] px-3 py-1 text-xs font-black text-white">
          {plan.badge}
        </span>
      )}
      ...
      <p className="mt-6 text-sm font-black uppercase tracking-[0.16em] text-teal-800">
        {plan.eyebrow}
      </p>

      <h3 className="mt-2 text-2xl font-black tracking-tight text-[#06183d]">
        {plan.name}
      </h3>
      ...
      <div className="mt-6">
        <p className="text-4xl font-black tracking-tight text-[#06183d]">
          {plan.price}
        </p>
      ...
      <Link
        href={plan.href}
        className={[
          "mt-6 inline-flex h-12 items-center justify-center gap-2 rounded-xl px-5 text-sm font-black transition",
          plan.highlighted
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        <p className="text-sm font-black text-[#06183d]">{plan.bestFor}</p>
      </div>
  ...
  return (
    ...
    >
      <div className="flex gap-4">
        ...
        <div>
          <h3 className="font-black leading-6 text-[#06183d]">{card.title}</h3>
          <p className="mt-2 text-sm leading-6 text-slate-600">{card.text}</p>

          <span className="mt-4 inline-flex items-center gap-1 text-sm font-black text-teal-800">
            {card.action}
...
function IncludedCard({
  ...
  return (
    <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <h3 className="mt-5 font-black text-[#06183d]">{item.title}</h3>
